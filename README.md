...
1) The first server compiles and outputs the webpack file (running on port 3000)
2) The second server is our app server. This is what you interact directly with, it runs on port 3003.

- The webpack server was originally configured with Create React App (and then ejected). It watches your code by default
- The app server uses babel, but not webpack. It uses nodemon to automatically restart.
- - You can also restart from the console by typing `rs`

## Debugging tools:

- React chrome extension [download](https://chrome.google.com/webstore/detail/react-developer-tools/fmkadmapgofadopljbjfkapdkoienihi)
- Redux chrome extension [download](https://chrome.google.com/webstore/detail/redux-devtools/lmhkpmbekcpmknklioeibfkpmmfibljd)
...
* `src/browser-entry-point.js` is the JavaScript entry point.
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/browser-entry-point.js`, but you could always remove that import if you later migrate to a different build tool.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your ```src/browser-entry-point.js``` file:
...
from [`src/browser-entry-point.js`](src/browser-entry-point.js).

If you had previously enabled service workers in your production deployment and
have decided that you would like to disable them for all your existing users,
you can swap out the call to `registerServiceWorker()` in
[`src/browser-entry-point.js`](src/browser-entry-point.js) first by modifying the service worker import:
```javascript
...
* If the watcher doesn’t see a file called `browser-entry-point.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebookincubator/create-react-app/issues/1164) due to a Webpack bug.
